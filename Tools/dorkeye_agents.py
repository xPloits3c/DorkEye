...
def _safe_json_local(raw: str, expected_type: type = dict):
    ...
    Handles: markdown fences, trailing commas, extra text surrounding JSON.
    """
    import re as _re
...
class BaseAgent(ABC):
    ...
    def _call_llm(self, prompt: str, max_tokens: int = 700) -> str:
        ...
        if not self.plugin:
            raise RuntimeError(f"[{self.name}] LLM not available — use regex mode")
...
class TriageAgent(BaseAgent):
    ...
    def run(self, results: List[dict], use_llm: bool = True) -> List[dict]:
        ...
        _log(
            f"[{self.name}] Triage {len(results)} results "
        ...
        # Calculate final score and clean up temporary fields
        for r in results:
    ...
    def _llm_score_batch(self, batch: List[dict], b_idx: int) -> None:
        ...
        prompt = (
            "You are an OSINT analyst. Assign an OSINT score 0-100 to each result.\n\n"
            "CRITERIA:\n"
            "  90-100 CRITICAL — credentials, DB admin, shell, exposed configs\n"
            "  70-89  HIGH     — admin panels, .env, .git, backups, config files\n"
            "  50-69  MEDIUM   — directory listing, vulnerable CMS, interesting login page\n"
            "  20-49  LOW      — standard login, generic pages\n"
            "   0-19  SKIP     — irrelevant\n\n"
            f"RESULTS:\n" + "\n".join(lines) + "\n\n"
            "Reply ONLY with a JSON array (one element per result, same order):\n"
            '[{"id":0,"score":85,"label":"HIGH","reason":"phpMyAdmin panel exposed"},...]\n###'
        )
        try:
    ...
    def _regex_score(result: dict) -> Tuple[int, List[str]]:
        ...
        # ── Runtime bonus from data already present in the result (v4.8) ───────
...
class _ScriptStyleStripper(_HTMLParser):
    ...
    Replaces the bypassable regex (CWE-20/116/185/186) with the built-in
    parser that correctly handles all syntactically valid variants of the
...
# Fallback regex — used ONLY if HTMLParser raises an exception on severely
...
#               such as `</script anything>` or `</  script  foo>` (spaces after
...
def _strip_page_content(text: str) -> str:
    """Removes script/style tags using HTMLParser, then cleans remaining HTML tags."""
...
class PageFetchAgent(BaseAgent):
    ...
    Instead of analyzing DDG snippets (100-200 characters, often useless),
    ...
    def run(self, results: List[dict]) -> List[dict]:
        ...
        Modifies results in-place adding 'page_content'.
    ...
    def _fetch(self, url: str) -> str:
        """Downloads and returns page text (truncated to max_chars).
        v4.8: retry max 2, UA rotation, saves response_headers in the result.
        """
...
class SecretsAgent(BaseAgent):
    """
    Analyzes the real page content (page_content) for credentials,
    API keys, DB connections, and other sensitive data.
    ...
    def run(self, results: List[dict]) -> List[dict]:
        ...
        """
        # Work only on results with content (page_content or non-empty snippet)
        candidates = [
            r for r in results
        ...
        for r in candidates:
            ...
            # Phase 2: LLM scan (only if LLM active and content is rich enough)
    ...
    def _regex_scan(self, content: str, url: str) -> List[dict]:
        findings = []
        # Dedup by normalized value (avoids duplicates for the same repeated key)
        seen_values: set = set()
        ...
        for category, pattern, description, has_group in _SECRET_PATTERNS:
            for match in pattern.finditer(content):
                ...
                # Normalized dedup: lowercase + strip whitespace
                ...
                # Severity from centralized mapping
    ...
    def _llm_scan(self, content: str, url: str) -> List[dict]:
        prompt = (
            "You are a security expert. Analyze this text and find ALL secrets or sensitive data.\n\n"
            f"SOURCE: {url}\n"
            f"CONTENT:\n{content[:3000]}\n\n"
            "Look for: API keys, tokens, passwords, hashes, JWTs, DB connections, cloud credentials, SSH keys.\n"
...
class ReportAgent(BaseAgent):
    ...
    def run(
        ...
        Args:
            ...
            analysis:    dict from llm_plugin.analyze_results() (optional)
            target:      description of the original target
            output_path: save path (None = do not save)
            fmt:         "html" | "md" | "json"
            extra:       dict with emails, pii, subdomains, cve_dorks (v3.0)

        Returns:
            String of the report in the requested format.
        """
...
# Pattern for outdated versions in headers
...
class HeaderIntelAgent(BaseAgent):
    """
    Analyzes HTTP response headers saved by PageFetchAgent.

    For each result with 'response_headers' it detects:
    ...
    Makes no HTTP requests — works on data already present.
    """
    ...
    def run(self, results: List[dict]) -> List[dict]:
        ...
        _log(f"[{self.name}] Analyzing headers for {len(candidates)} results...", style="bold cyan")
    ...
    def _analyze(self, headers: dict, url: str) -> dict:
        ...
        # Suspicious cache / debug headers
...
# Technology signatures: (pattern, tech_name, category)
...
# Targeted CVE dorks per technology (feeds DorkCrawlerAgent)
...
class EmailHarvesterAgent(BaseAgent):
    """
    Collects and categorizes email addresses from snippet + page_content.

    Categories: admin, security, info, noreply, personal (everything else).
    Produces 'emails_found' (List[dict]) per result and a global list
...
class PiiDetectorAgent(BaseAgent):
    ...
    Types detected: email, phone (IT/EU/US), IBAN, Italian tax code (CF),
    ...
    def _scan(self, content: str, url: str) -> List[dict]:
        ...
        for category, pattern, description, has_group in _PII_PATTERNS:
            for match in pattern.finditer(content):
                ...
                # Special Luhn validation for credit cards
...
class SubdomainHarvesterAgent(BaseAgent):
    ...
    For each detected base domain it collects unique subdomains, deduplicates
    them, and generates 'site:sub.domain.*' dorks ready to be injected into
...
# Dork templates for detected CMS/technologies
...
# Templates for interesting paths/directories found
...
# Templates for sensitive file extensions found
...
# Patterns to identify technologies from snippets/titles/URLs
...
# Paths considered "interesting" for follow-up
...
class DorkCrawlerAgent(BaseAgent):
    ...
    Executes multiple rounds of dorking, automatically refining the dorks
    ...
    Args:
        ...
        min_new_high:   min. new HIGH/CRITICAL to continue (default: 1)
        delay_between:  pause in seconds between searches (default: 3.0)
        stealth:        slow down further to avoid rate limiting (default: False)
    """
    ...
    def run(
        ...
        """
        Executes the adaptive recursive crawl.

        Args:
            seed_dorks: list of starting dorks (round 1)
            target:     description of the target (for logging)
        ...
        for round_n in range(1, self.max_rounds + 1):
            ...
            # ── Stop condition: no new HIGH/CRITICAL after round 1 ─────────
            ...
            # ── Last round: do not generate more dorks ─────────────────────
            ...
            if not next_dorks:
                _log(f"[{self.name}] No new dorks can be generated — stop.", style="yellow")
                stop_reason = "no_new_dorks"
                break
            ...
            if round_n < self.max_rounds:
                _log(
                    f"[{self.name}] Pause {self.delay_between:.1f}s "
                    f"(stealth: {'on' if self.stealth else 'off'})...",
                    style="dim"
                )
    ...
    def _extract_intelligence(self, results: List[dict]) -> dict:
        ...
        Analyzes URLs, titles and snippets of all results (prioritizing
        HIGH/CRITICAL) to build an intelligence map that guides the
        generation of follow-up dorks.

        Returns:
            dict with: domains, paths, technologies, extensions, params
        """
        intelligence: dict = {
            "domains":      set(),
            "paths":        set(),
    ...
    def _generate_followup_dorks(self, intelligence: dict) -> List[str]:
        ...
        domains     = intelligence.get("domains", [])[:8]   # max 8 domains
        paths       = intelligence.get("paths", [])[:5]     # max 5 paths
        technologies = intelligence.get("technologies", [])
        extensions  = intelligence.get("extensions", [])
        params      = intelligence.get("params", [])[:4]    # max 4 params
        ...
        for path in paths:
            for domain in domains[:4]:  # max 4 domains per path
                for tmpl in _PATH_DORK_TEMPLATES[:3]:  # max 3 templates per path
        ...
        for ext in extensions:
            ...
            for tmpl in templates[:2]:  # max 2 templates per ext
    ...
    def _log_intelligence(self, intel: dict, round_n: int) -> None:
        ...
        if intel.get("domains"):
            parts.append(f"domains:{len(intel['domains'])}")
...
def add_crawler_args(parser) -> object:
    ...
    g.add_argument(
        "--crawl",
        action="store_true",
        help="Enable adaptive recursive crawl after the initial search",
    )
    g.add_argument(
        "--crawl-rounds",
        type=int, default=4,
    ...
    g.add_argument(
        "--crawl-report",
        action="store_true",
        help="Generate HTML report of the crawl at the end",
    )
    g.add_argument(
        "--crawl-out",
        type=str, default=None,
        help="Crawl report path (default: dorkeye_crawl_<timestamp>.html)",
    )
    return parser
...
def run_crawl(
    ...
    """
    Entry point for dorkeye.py — starts DorkCrawlerAgent.

    Args:
        seed_dorks: initial dorks (from -d or --dg)
        args:       argparse.Namespace with --crawl-* flags
        target:     target description
        plugin:     optional LLMProvider

    Returns:
        dict crawler output (results, rounds, dorks_used, etc.)
    """
...
def run_analysis_pipeline(
    ...
    """
    Executes the post-search analysis pipeline v3.0.
    ...
    Works WITHOUT LLM (llm_plugin=None):
        ...
        3.  HeaderIntelAgent        — analyzes response headers
        4.  TechFingerprintAgent    — detects CMS/framework/versions
        5.  SecretsAgent            — secrets scan + hash + severity
        6.  PiiDetectorAgent        — detects PII (CC, IBAN, CF, SSN, etc.)
        7.  EmailHarvesterAgent     — collects and categorizes emails
    ...
    Args:
        results:    list of DorkEye results
        llm_plugin: DorkEyeLLMPlugin (optional — None = regex only)
        args:       argparse.Namespace with --analyze-* flags
        target:     target description (for the report)

    Returns:
        dict with: triaged, secrets_total, pii_total, emails_total,
    ...
    if not results:
        _log("[Agents] No results to analyze.", style="yellow")
        return output
...
# ══════════════════════════════════════════════════════════════════════════════
#  STANDALONE — analyze a JSON file of DorkEye results
# ══════════════════════════════════════════════════════════════════════════════
...
if __name__ == "__main__":
    ...
    parser = _ap.ArgumentParser(
        description="DorkEye Agents v3.0 — Autonomous analysis on results file",
        formatter_class=_ap.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  # Autonomous analysis (no LLM required)\n"
    ...
    parser.add_argument(
        "results_file",
        help="DorkEye results JSON file (produced with -o results.json)",
    )
    ...
    try:
        ...
        parser.add_argument("--llm", action="store_true",
                            help="(not available: dorkeye_llm_plugin.py missing)")
    ...
    if args.llm and _llm_available:
        ...
        if not llm:
            print("[!] LLM not initialized — continuing in autonomous mode.", file=sys.stderr)
    elif args.llm and not _llm_available:
        print("[!] dorkeye_llm_plugin.py not found — autonomous mode.", file=sys.stderr)

    print(f"\n[*] Analyzing {len(results)} results from '{p.name}'")
